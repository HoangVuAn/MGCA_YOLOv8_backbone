...
class MultimodalPretrainingDataset(data.Dataset):
    ...
    def load_text_data(self, split):
        ...
        #file name: file data of split
        #path2sent: all file name + sent
    ...
    def get_caption(self, path):
        ...
        #chieu dai chi tinh cac gia tri khac 0
...
class MultimodalPretrainingDatasetEndo(data.Dataset):
    def __init__(self, split="train", transform=None, data_pct=1.0,
                 imsize=640, max_words=112, sent_num=3):
        super().__init__()
        if not os.path.exists(ENDO_DATA_DIR):
            raise RuntimeError(f"{ENDO_DATA_DIR} does not exist!")

        self.transform = transform
        self.imsize = imsize
        self.df = pd.read_excel(ENDO_MASTER_CSV)
        # lay path goc
        # self.df[ENDO_PATH_COL] = self.df[ENDO_PATH_COL].apply(
        #     lambda x: os.path.join(ENDO_DATA_DIR, "UTDD" if self.df["Loại tổn thương"]=="Ung thư dạ dày" else "UTTQ"))

        def add_directory_path(row):
            endo_data_dir = ENDO_DATA_DIR
            if row["disease"] == "Ung thư dạ dày":
                endo_data_dir = os.path.join(endo_data_dir, "UTDD/images")
            else:
                endo_data_dir = os.path.join(endo_data_dir, "UTTQ/images")
            return os.path.join(endo_data_dir, f"{row[ENDO_PATH_COL]}.jpeg")
        

        self.df[ENDO_PATH_COL] = self.df.apply(add_directory_path, axis=1)
        # load studies and study to text mapping
        self.filenames, self.path2sent = self.load_text_data(split)
        
        self.df = self.df[self.df[ENDO_SPLIT_COL] == split]
        if data_pct == 1.0 and split == "train":
            self.df = self.df.sample(frac=data_pct, random_state=42)
        self.df.reset_index(drop=True, inplace=True)
    ...
    def load_text_data(self, split):
        ...
        filenames = []
        for row in self.df.itertuples():
            cur_split = getattr(row, ENDO_SPLIT_COL)
            path = getattr(row, ENDO_PATH_COL)
            if cur_split == split and path in path2sent:
                filenames.append(path)
    ...
    def create_path_2_sent_mapping(self):
        ...
        for _, row in tqdm(self.df.iterrows(), total=self.df.shape[0]):
            ...
            if cnt >= 3:
                sent_lens.append(cnt)
                num_sents.append(len(study_sent))
                path2sent[row[ENDO_PATH_COL]] = study_sent
...
if __name__ == "__main__":
    ...
    dataset = MultimodalPretrainingDatasetEndo(split="train", transform=transform)
    data = dataset[0]
    print(data)
